...
def select_refactor_target(
    ...
    *,
    ...
    if not candidates:
        ...
        # Filter to only items that actually need repair (rate < 1.0)
        if target == "program":
            repair_needed = [i for i, rate in enumerate(prog_rates) if rate < 1.0]
        else:
            repair_needed = [j for j, rate in enumerate(test_rates) if rate < 1.0]
        
        if not repair_needed:
            print("⚠️  No items need repair - all have perfect scores!")
            return None, None
        
        idx = random.choice(repair_needed)
        return target, idx
